...
apply_task_to_project_backlog() {
  ...
  for attempt in 1 2 3 4 5; do
    if set_out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "ISSUE-${issue_number}" "Backlog" "Backlog" 2>&1)"; then
      echo "$set_out"
      return 0
    fi
    rc=$?
    ...
    if [[ "$rc" -eq 75 ]]; then
      ...
      runtime_out="$("${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" enqueue "ISSUE-${issue_number}" "Backlog" "Backlog" "backlog-seed:${issue_number}" 2>&1 || true)"
      while IFS= read -r line; do
        [[ -z "$line" ]] && continue
        echo "$line"
      done <<< "$runtime_out"
