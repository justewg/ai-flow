...
if [[ "$pending_post" == "1" ]]; then
  outbox_count="0"
  if outbox_out="$("${ROOT_DIR}/.flow/scripts/github_outbox.sh" count 2>/dev/null)"; then
...
if [[ "$reply_mode" == "QUESTION" ]]; then
  ...
  if ! answer_out="$("${ROOT_DIR}/.flow/scripts/gh_retry.sh" gh api "repos/${REPO}/issues/${issue_number}/comments" -f body="$answer_body" 2>&1)"; then
    rc=$?
    if [[ "$rc" -eq 75 ]]; then
      tmp_answer="$(mktemp "${CODEX_DIR}/answer_body.XXXXXX")"
      trap 'rm -f "$tmp_answer"' EXIT
      printf '%s\n' "$answer_body" > "$tmp_answer"
      if queue_out="$("${ROOT_DIR}/.flow/scripts/github_outbox.sh" enqueue_issue_comment "$REPO" "$issue_number" "$tmp_answer" "$task_id" "ANSWER" "0" 2>&1)"; then
...
if ! ack_out="$("${ROOT_DIR}/.flow/scripts/gh_retry.sh" gh api "repos/${REPO}/issues/${issue_number}/comments" -f body="$ack_body" 2>&1)"; then
  rc=$?
  if [[ "$rc" -eq 75 ]]; then
    tmp_ack="$(mktemp "${CODEX_DIR}/ack_body.XXXXXX")"
    trap 'rm -f "$tmp_ack"' EXIT
    printf '%s\n' "$ack_body" > "$tmp_ack"
    if queue_out="$("${ROOT_DIR}/.flow/scripts/github_outbox.sh" enqueue_issue_comment "$REPO" "$issue_number" "$tmp_ack" "$task_id" "ACK" "0" 2>&1)"; then
      while IFS= read -r line; do
        [[ -z "$line" ]] && continue
        echo "$line"
      done <<< "$queue_out"
