...
refresh_daemon_github_token() {
  ...
  if token="$("${ROOT_DIR}/.flow/scripts/gh_app_auth_token.sh" 2>"$auth_log_file")"; then
...
notify_github_runtime_if_needed() {
  ...
  if (( applied_count > 0 )); then
    ...
    if notify_out="$("${ROOT_DIR}/.flow/scripts/telegram_local_notify.sh" "$msg_file" 2>&1)"; then
      while IFS= read -r line; do
        [[ -z "$line" ]] && continue
...
while true; do
  ...
  runtime_apply_out="$("${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" apply 5 2>&1 || true)"
  ...
  if output="$(DAEMON_LOOP_PUSH_REMOTE=1 "${ROOT_DIR}/.flow/scripts/daemon_tick.sh" 2>&1)"; then
    while IFS= read -r line; do
      [[ -z "$line" ]] && continue
      log "$line"
    done <<<"$output"
