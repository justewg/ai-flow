...
push_remote_status_if_needed() {
  local push_out rc
  if push_out="$("${ROOT_DIR}/.flow/scripts/ops_remote_status_push.sh" 2>&1)"; then
    while IFS= read -r line; do
...
push_remote_summary_if_needed() {
  local push_out rc
  if push_out="$("${ROOT_DIR}/.flow/scripts/ops_remote_summary_push.sh" 2>&1)"; then
    while IFS= read -r line; do
...
enqueue_project_status_runtime() {
  ...
  if runtime_out="$("${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" enqueue "$target" "$status_name" "$flow_name" "$reason" 2>&1)"; then
...
run_gh_retry_capture() {
  ...
  if out="$("${ROOT_DIR}/.flow/scripts/gh_retry.sh" "$@" 2>"$err_file")"; then
    if [[ -s "$err_file" ]]; then
      cat "$err_file" >&2
    fi
    rm -f "$err_file"
...
maybe_release_active_task_on_status_mismatch() {
  ...
  if [[ "$active_issue_number" =~ ^[0-9]+$ ]]; then
    if issue_has_auto_ignore_label "$active_issue_number"; then
      ...
      "${ROOT_DIR}/.flow/scripts/executor_reset.sh" >/dev/null || true
      : > "$active_task_file"
      : > "$active_item_file"
      : > "$active_issue_file"
...
set_dirty_gate_project_status() {
  ...
  if status_out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "$status_target" "$status_name" "$flow_name" 2>&1)"; then
...
auto_commit_dirty_worktree() {
  ...
  if commit_out="$("${ROOT_DIR}/.flow/scripts/dev_commit_push.sh" "$commit_message" "${stage_paths[@]}" 2>&1)"; then
...
maybe_process_dirty_gate_reply() {
  ...
  if ! reply_probe_out="$("${ROOT_DIR}/.flow/scripts/daemon_check_replies.sh" 2>&1)"; then
    local rc=$?
    if [[ "$rc" -eq 75 ]]; then
      echo "WAIT_GITHUB_API_UNSTABLE=1"
...
if backlog_seed_out="$("${ROOT_DIR}/.flow/scripts/backlog_seed_apply.sh" 2>&1)"; then
...
outbox_out="$("${ROOT_DIR}/.flow/scripts/github_outbox.sh" flush 2>&1 || true)"
...
pending_actions="$(
  "${ROOT_DIR}/.flow/scripts/github_outbox.sh" count 2>/dev/null |
...
runtime_status_out="$("${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" apply 5 2>&1 || true)"
...
reply_probe_out="$("${ROOT_DIR}/.flow/scripts/daemon_check_replies.sh" 2>&1)"
while IFS= read -r line; do
  [[ -z "$line" || "$line" == "NO_WAITING_USER_REPLY=1" ]] && continue
  echo "$line"
done <<< "$reply_probe_out"
...
if [[ "$stale_reason" == "REVIEW_PR_MERGED" || "$stale_reason" == "ISSUE_CLOSED" ]]; then
  if [[ "$stale_issue_number" =~ ^[0-9]+$ ]]; then
    ...
    if stale_status_out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "$stale_status_target" "Done" "Done" 2>&1)"; then
...
if printf '%s' "$reply_probe_out" | grep -q '^USER_REPLY_RECEIVED=1'; then
  ...
  if is_review_feedback_kind "$reply_kind"; then
    ...
    if status_out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "$status_target" "$target_status" "$target_flow" 2>&1)"; then
      emit_lines "$status_out"
    else
      rc=$?
      emit_lines "$status_out"
    ...
    "${ROOT_DIR}/.flow/scripts/executor_reset.sh" >/dev/null
    ...
    exec_out="$("${ROOT_DIR}/.flow/scripts/executor_tick.sh" "$review_task_id" "$review_issue_number" 2>&1)"
    emit_lines "$exec_out"
    exit 0
  fi
fi
...
if [[ -s "${CODEX_DIR}/daemon_active_task.txt" ]]; then
  ...
  if [[ -n "$active_issue_number" ]]; then
    exec_out="$("${ROOT_DIR}/.flow/scripts/executor_tick.sh" "$active_task_id" "$active_issue_number" 2>&1)"
    emit_lines "$exec_out"
  else
...
if health_out="$("${ROOT_DIR}/.flow/scripts/github_health_check.sh" 2>&1)"; then
  :
else
  rc=$?
...
if (( skip_sync_branches == 0 )); then
  if sync_out="$("${ROOT_DIR}/.flow/scripts/sync_branches.sh" 2>&1)"; then
    :
  else
    rc=$?
    emit_lines "$sync_out"
...
if status_out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "$item_id" "$target_status" "$target_flow" 2>&1)"; then
  emit_lines "$status_out"
else
  rc=$?
  emit_lines "$status_out"
