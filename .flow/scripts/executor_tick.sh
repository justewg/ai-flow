...
runtime_status_out="$("${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" apply 3 2>&1 || true)"
...
start_out="$("${ROOT_DIR}/.flow/scripts/executor_start.sh" "$task_id" "$issue_number" 2>&1)"
while IFS= read -r line; do
  [[ -z "$line" ]] && continue
  echo "$line"
done <<<"$start_out"
