...
before_status="$("${ROOT_DIR}/.flow/scripts/gh_app_auth_pm2_status.sh" --require-online)"
...
while (( SECONDS < deadline )); do
  sleep 1
  if after_status="$("${ROOT_DIR}/.flow/scripts/gh_app_auth_pm2_status.sh" 2>/dev/null)"; then
    ...
    if [[ "${after_state}" == "online" ]] \
      ...
      "${ROOT_DIR}/.flow/scripts/gh_app_auth_health.sh" >/dev/null
      echo "AUTH_HEALTH_OK"
      exit 0
    fi
  fi
...
"${ROOT_DIR}/.flow/scripts/gh_app_auth_pm2_status.sh" || true
exit 1
