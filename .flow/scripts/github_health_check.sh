...
probe_out=""
if probe_out="$(GH_RETRY_MAX_ATTEMPTS="$max_attempts" GH_RETRY_BASE_SLEEP_SEC="$base_sleep_sec" "${ROOT_DIR}/.flow/scripts/gh_retry.sh" gh api rate_limit --jq '.rate.remaining' 2>&1)"; then
