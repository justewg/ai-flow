...
run_gh_retry_capture() {
  local out=""
  local rc=0
  if out="$("${ROOT_DIR}/.flow/scripts/gh_retry.sh" "$@" 2>&1)"; then
    printf '%s' "$out"
    return 0
  fi
  rc=$?
...
ensure_project_backlog() {
  ...
  if out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "ISSUE-${issue_number}" "Backlog" "Backlog" 2>&1)"; then
    :
  else
    rc=$?
    if [[ "$rc" -eq 75 ]] || printf '%s' "$out" | grep -Eiq 'rate limit|api\.github\.com|could not resolve|timeout|connection reset|failed to connect'; then
      "${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" enqueue "ISSUE-${issue_number}" "Backlog" "Backlog" "issue-285-reframe:${issue_number}" >/dev/null 2>&1 || true
...
4. Зарегистрировать webhook:
   - через wrapper: `.flow/scripts/run.sh ops_bot_webhook_register register`
   - напрямую: `.flow/scripts/ops_bot_webhook_register.sh register`
   - только проверка: `.flow/scripts/ops_bot_webhook_register.sh info`
...
- reload nginx;
- остановить ops bot: `.flow/scripts/run.sh ops_bot_pm2_stop`.
