...
Examples:
  .flow/scripts/onboarding_audit.sh
  .flow/scripts/onboarding_audit.sh --profile acme
  .flow/scripts/onboarding_audit.sh --profile acme --skip-network
EOF
}
...
check_required_executable ".flow/scripts/run.sh" "TOOLKIT_RUN_SH" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/profile_init.sh" "TOOLKIT_PROFILE_INIT" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/onboarding_audit.sh" "TOOLKIT_ONBOARDING_AUDIT" "Перенеси обновлённый toolkit .flow/ в новый repo."
check_required_executable ".flow/scripts/create_migration_kit.sh" "TOOLKIT_CREATE_MIGRATION_KIT" "Перенеси обновлённый toolkit .flow/ в новый repo."
check_required_executable ".flow/scripts/apply_migration_kit.sh" "TOOLKIT_APPLY_MIGRATION_KIT" "Перенеси обновлённый toolkit .flow/ в новый repo."
check_required_executable ".flow/scripts/daemon_install.sh" "TOOLKIT_DAEMON_INSTALL" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/watchdog_install.sh" "TOOLKIT_WATCHDOG_INSTALL" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/gh_app_auth_token.sh" "TOOLKIT_AUTH_TOKEN" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/telegram_local_notify.sh" "TOOLKIT_TG_NOTIFY" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/ops_bot_webhook_register.sh" "TOOLKIT_OPS_WEBHOOK" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/ops_remote_status_push.sh" "TOOLKIT_OPS_REMOTE_STATUS_PUSH" "Перенеси весь каталог .flow/ в новый repo."
check_required_executable ".flow/scripts/ops_remote_summary_push.sh" "TOOLKIT_OPS_REMOTE_SUMMARY_PUSH" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/env/resolve_config.sh" "TOOLKIT_RESOLVE_CONFIG" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/gh_app_auth_service.js" "TOOLKIT_AUTH_SERVICE" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/gh_app_auth_pm2_ecosystem.config.cjs" "TOOLKIT_AUTH_PM2_CONFIG" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/status_snapshot.sh" "TOOLKIT_STATUS_SNAPSHOT" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/log_summary.sh" "TOOLKIT_LOG_SUMMARY" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/ops_bot_service.js" "TOOLKIT_OPS_BOT_SERVICE" "Перенеси весь каталог .flow/ в новый repo."
check_required_file ".flow/scripts/ops_bot_pm2_ecosystem.config.cjs" "TOOLKIT_OPS_BOT_PM2_CONFIG" "Перенеси весь каталог .flow/ в новый repo."
check_required_file "COMMAND_TEMPLATES.md" "COMMAND_TEMPLATES" "Перенеси COMMAND_TEMPLATES.md в корень consumer-project."
check_required_file ".flow/docs/flow-onboarding-checklist.md" "DOC_ONBOARDING_CHECKLIST" "Перенеси обязательные onboarding docs в .flow/docs/."
check_required_file ".flow/docs/flow-onboarding-quickstart.md" "DOC_ONBOARDING_QUICKSTART" "Перенеси обязательные onboarding docs в .flow/docs/."
check_required_file ".flow/docs/flow-portability-runbook.md" "DOC_FLOW_PORTABILITY" "Перенеси обязательные onboarding docs в .flow/docs/."
check_required_file ".flow/docs/gh-app-daemon-integration-plan.md" "DOC_GH_APP_PLAN" "Перенеси обязательные onboarding docs в .flow/docs/."
check_required_file ".flow/docs/ops-bot-dashboard.md" "DOC_OPS_BOT_DASHBOARD" "Перенеси обязательные automation docs в .flow/docs/."
...
if [[ -f "$required_repo_actions_files_manifest" ]]; then
  ...
  while IFS= read -r repo_action_rel_path; do
    ...
    if [[ -f "${ROOT_DIR}/${repo_action_rel_path}" ]]; then
      ...
      report_action "REPO_ACTION_FILE" "Разверни repo automation overlay через .flow/scripts/run.sh apply_migration_kit --project <name> или вручную восстанови ${repo_action_rel_path}."
    fi
...
if [[ -z "$profile" && -z "$env_file" ]]; then
  ...
  report_action "PROFILE" "Для полной проверки profile/env запусти: .flow/scripts/run.sh onboarding_audit --profile <name>"
  ...
  if [[ -n "$state_dir" ]]; then
    if [[ -d "$state_dir" ]]; then
      ...
      report_warn "PROFILE_STATE_DIR" "missing:${state_dir}"
      report_action "PROFILE_STATE_DIR" "Создай state-dir через: .flow/scripts/run.sh profile_init init --profile ${profile}"
    fi
  fi
