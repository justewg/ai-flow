...
SERVICE_FILE="${ROOT_DIR}/.flow/scripts/ops_bot_service.js"
...
usage() {
  cat <<'EOF'
Usage:
  .flow/scripts/ops_bot_post_smoke_check.sh [--manual-file <path>] [--report-file <path>] [--no-report]
...
check_webhook_info() {
  ...
  set +e
  output="$("${ROOT_DIR}/.flow/scripts/ops_bot_webhook_register.sh" info 2>&1)"
  rc=$?
  set -e
