...
if (( open_pr_count == 0 )); then
  create_out="$("${ROOT_DIR}/.flow/scripts/pr_create.sh" "$tmp_title" "$tmp_body")"
  ...
  "${ROOT_DIR}/.flow/scripts/pr_edit.sh" "$pr_number" "$tmp_title" "$tmp_body" >/dev/null
...
if final_status_out="$("${ROOT_DIR}/.flow/scripts/project_set_status.sh" "$status_target" "$FINAL_STATUS" "$FINAL_FLOW" 2>&1)"; then
  ...
  if [[ "$final_status_rc" -eq 75 ]] || printf '%s' "$final_status_out" | grep -Eiq \
    ...
    runtime_out="$("${ROOT_DIR}/.flow/scripts/project_status_runtime.sh" enqueue "$status_target" "$FINAL_STATUS" "$FINAL_FLOW" "task-finalize:${task_id}" 2>&1 || true)"
