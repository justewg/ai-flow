...
daemon_loop_running() {
  ...
  pgrep -f "${ROOT_DIR}/.flow/scripts/daemon_loop.sh" >/dev/null 2>&1
}
...
EOF
  local out rc
  if out="$("${ROOT_DIR}/.flow/scripts/telegram_local_notify.sh" "$msg_file" 2>&1)"; then
    while IFS= read -r line; do
      [[ -z "$line" ]] && continue
      log "TG_NOTIFY: $line"
    done <<< "$out"
...
run_soft_tick() {
  local out rc
  if out="$("${ROOT_DIR}/.flow/scripts/daemon_tick.sh" 2>&1)"; then
    emit_lines "$out"
    return 0
  fi
  rc=$?
...
run_medium_recovery() {
  ...
  if out="$("${ROOT_DIR}/.flow/scripts/executor_reset.sh" 2>&1)"; then
    emit_lines "$out"
  else
    rc=$?
    emit_lines "$out"
...
run_hard_recovery() {
  ...
  if out="$("${ROOT_DIR}/.flow/scripts/daemon_uninstall.sh" "$DAEMON_LABEL" 2>&1)"; then
    emit_lines "$out"
  else
    rc=$?
    emit_lines "$out"
  ...
  if out="$("${ROOT_DIR}/.flow/scripts/daemon_install.sh" "$DAEMON_LABEL" "$DAEMON_INTERVAL" 2>&1)"; then
    emit_lines "$out"
  else
    local install_rc=$?
    emit_lines "$out"
  ...
  if out="$("${ROOT_DIR}/.flow/scripts/daemon_status.sh" "$DAEMON_LABEL" 2>&1)"; then
    emit_lines "$out"
  else
    local status_rc=$?
    emit_lines "$out"
...
daemon_agent_status_out="$("${ROOT_DIR}/.flow/scripts/daemon_status.sh" "$DAEMON_LABEL" 2>&1 || true)"
