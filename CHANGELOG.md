...
- `[APP-07]` Общий flow-toolkit физически вынесен в `/var/sites/.ai-flow/shared/{scripts,docs}`; в `PLANKA` оставлены только symlink-и `.flow/scripts` и `.flow/docs`, чтобы следующий шаг к отдельному git-repo/submodule делался уже от общего host-level каталога.
