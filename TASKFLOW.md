...
- Источник правды по очереди задач: GitHub Project (поля `Task ID`, `Priority`, `Status`, `Flow`).
...
- GitHub Actions после merge PR в `main`:
  - извлекает `PL-xxx` из title/body PR;
...
9. Не запускать подхват задач при грязном локальном рабочем дереве (wait-state), чтобы не конфликтовать с ручной работой в чате.
...
2. Авто-режим демона:
   ...
   - демон отвечает за подхват очереди, синхронизацию, перевод в `In Progress` и старт рабочего цикла.
...
- содержательная разработка и коммуникация остаются в чате;
- завершение дельты оформляется одной командой `task_finalize`.

## 6. Политика языка и оформления
- Общение: русский.
- Коммиты: русский формат `PL-xxx: Краткое русское описание`.
- Заголовки PR: русский формат `PL-xxx Краткое русское описание`.
