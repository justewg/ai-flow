...
Правила:
1. Новая задача: `Status=Backlog`, `Flow=Backlog`.
2. Твоя команда на старт: перевести задачу `Status: Backlog -> Todo`.
3. Daemon мониторит только `Status=Todo`.
...
- `Backlog`: хранилище всех задач/черновиков до запуска.
- `Todo`: входная очередь для демона (твой явный сигнал "запускай").
...
11.1. Если задача уже в `Status=Review`, daemon включает режим `WAIT_REVIEW_FEEDBACK`:
   ...
   - daemon классифицирует комментарий:
     - `QUESTION` — публикует `CODEX_SIGNAL: AGENT_ANSWER` и остается в `WAIT_REVIEW_FEEDBACK`;
     - `REWORK` — публикует `CODEX_SIGNAL: AGENT_RESUMED_REVIEW`, переводит задачу `Review -> In Progress` и возобновляет executor (`REVIEW_FEEDBACK_RESUMED`).
   - явный override в комментарии:
     - `CODEX_MODE: QUESTION`
     - `CODEX_MODE: REWORK`
...
2. Авто-режим демона:
   - фоновый `launchd`-процесс делает polling проекта и реагирует на `Status=Todo`;
...
- выбор задачи делает только ты (перемещением карточки в `Todo`);
...
- `Signal: AGENT_ANSWER` — агент дал текстовый ответ в Issue без запуска доработки.
...
3. Убедись, что есть опции: `Backlog`, `Todo`, `In Progress`, `Review`, `Done`, `Closed`.
4. Поставь `Backlog` значением по умолчанию для `Status` (если в UI доступен явный выбор default).
5. Сохрани изменения и расположи опции в порядке: `Backlog`, `Todo`, `In Progress`, `Review`, `Done`, `Closed`.
6. Если явного выбора default нет, оставь `Backlog` первым в списке опций `Status`.
7. В board-view убедись, что отображаются соответствующие колонки.

Практический смысл:
- это не GitHub Label, а значение single-select поля `Status` (семантическое состояние проекта);
- при перетаскивании карточки в колонку `Todo` значение поля `Status` обновляется автоматически.
...
- чтобы исключить автоподхват "не той" новой карточки при сетевой деградации, дефолт `Status` должен быть именно `Backlog`.
...
- Важно: передача `singleSelectOptions` в `updateProjectV2Field` перезаписывает весь список опций, поэтому нужно отправлять полный список актуальных опций.
