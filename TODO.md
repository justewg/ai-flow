...
| PL-056 | Добавить `docker-hosted` deployment mode для ai-flow как server-appliance сценарий | MVP | P1 | In Progress | Есть wizard/installer, который генерирует `docker-compose.yml`, `Dockerfile`, env/secrets layout и authoritative workspace для profile; compose-контур `runtime`, `gh-app-auth`, `daemon`, `watchdog`, `ops-bot` поднимается через `docker compose`, использует host OpenAI/VPN path и даёт воспроизводимый runtime без ручной сборки host-native зависимостей |
