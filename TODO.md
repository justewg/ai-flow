...
| PL-056 | Добавить `docker-hosted` deployment mode для ai-flow как server-appliance сценарий | MVP | P1 | In Progress | Есть wizard/installer, который генерирует `docker-compose.yml`, `Dockerfile`, env/secrets layout и authoritative workspace для profile; контейнеры `runtime`, `daemon`, `watchdog` поднимаются через `docker compose`, используют host OpenAI/VPN path и дают воспроизводимый runtime без ручной сборки host-native зависимостей |
