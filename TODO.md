...
| PL-061 | Добавить optional remote-agent access contour для внешней AI-диагностики | MVP | P0 | In Progress | Для self-hosted ai-flow есть отдельный user `aiflow`, forced-command SSH gateway, sudo allowlist только на read-only `remote_probe`, audit log и documented rollback; этот контур optional, но входит в канонический host bootstrap/runbook как способ удалённой диагностики без shell-доступа |
| PL-057 | Поднять host-level nginx ingress для ai-flow на `aiflow.ewg40.ru` | MVP | P0 | Planned | Публичный host-level контур для `ops-bot` опубликован через nginx на `aiflow.ewg40.ru` и отдает `/health`, `/ops/status`, `/ops/status.json`, `/ops/debug/*`, `/telegram/webhook`; `gh-app-auth` наружу не опубликован |
...
| PL-060 | Включить `env_audit` в эксплуатационный health-check и runbook | MVP | P1 | Planned | Для диагностики платформы штатно используются `/health`, `/ops/debug/runtime.json` и `env_audit`; конфиг-дрифт отражается в health/debug surface и описан в runbook без SSH-only reverse engineering |
