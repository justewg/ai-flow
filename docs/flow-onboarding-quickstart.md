...
Если нужен перенос двух проектов на одном хосте, multi-profile режим, rollback и extraction strategy, см. `.flow/shared/docs/flow-portability-runbook.md`.
Если нужен отдельный onboarding GitHub App auth-сервиса, см. `.flow/shared/docs/gh-app-daemon-integration-plan.md`.
Если нужен help по repo Actions secrets, см. `.flow/shared/docs/github-actions-repo-secrets.md`.
Если нужен пошаговый чеклист без лишнего контекста, см. `.flow/shared/docs/flow-onboarding-checklist.md`.
Если toolkit уже опубликован на этом хосте в shared-root, используй `<sites-root>/.ai-flow/shared` как источник submodule/shared-toolkit для нескольких consumer-project. Локальные runtime-links (`logs`, `tmp` и т.п.) создаёт владелец окружения или onboarding-скрипт и они не должны храниться в git самого проекта.

## Что считается результатом
После прохождения quickstart у вас есть:
- локальная папка проекта с каталогом `.flow/shared/scripts`;
...
Подробно про GitHub App, permissions и hybrid mode: `.flow/shared/docs/gh-app-daemon-integration-plan.md`.
...
- в текущем проекте: `.flow/shared/scripts/run.sh create_migration_kit --project acme --target-repo <HOME>/sites/acme-app`
...
- затем повторно прогнать `.flow/shared/scripts/run.sh onboarding_audit --profile acme`, чтобы audit проверил repo workflow overlay и наличие secrets через GitHub API
...
Если нет, исправляйте `CHECK_FAIL ...` в `.flow/config/flow.env`, а не исходники `.flow/shared/scripts`.
...
- `.flow/shared/docs/flow-portability-runbook.md` — multi-project, migration, rollback, extraction strategy.
- `.flow/shared/docs/gh-app-daemon-integration-plan.md` — GitHub App permissions, hybrid mode, auth-service onboarding.
- `.flow/shared/docs/flow-toolkit-packaging.md` — что переносить в consumer-project и как выносить toolkit в отдельный repo.
- `.flow/shared/scripts/README.md` — каталог команд и env-справочник.
