...
| `.flow/shared/scripts/run.sh profile_init orchestrate --profile <name>` | финальный apply/install/smoke этап после заполнения `flow.env` | готовый `flow.env`, known profile, optional `--skip-network` для audit | `AUDIT_SUMMARY`, `AUDIT_READY/AUDIT_WARN/AUDIT_BLOCKER/AUDIT_ACTION`, `ORCHESTRATION_STEP`, `FINAL_SUMMARY`, `RECOVERY_*`, `ORCHESTRATION_REPORT` | последовательный запуск `onboarding_audit`, `profile_init install`, `github_health_check`, `status_snapshot`, `profile_init preflight` и запись retry/rollback report |
...
| `.flow/shared/scripts/run.sh profile_init preflight --profile <name>` | финальная проверка и повторный rerun baseline | тот же profile/env, что и для install | `PREFLIGHT_READY=0|1`, `SMOKE_STEP`, `CHECKLIST`, `CHECK_OK/CHECK_FAIL` по runtime | health summary и smoke checklist; `PREFLIGHT_READY=1` только если env валиден и daemon/watchdog находятся в `RUNNING` |
