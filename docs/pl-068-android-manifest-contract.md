...
3. Конфиг-канал в MVP публикует полный config-файл, а не patch/diff.
4. Проверка обновлений запускается только вручную по явному действию пользователя.
5. Config update может применяться сразу после успешной проверки и валидации.
6. APK update никогда не ставится скрытно: только после явного подтверждения пользователя и через системный installer.
...
5. Дальше выполняется один из сценариев ниже.
