...
- `.flow/shared/scripts/provider_corpus_run.sh --issues "633 626 622" [--state-dir <path>] [--module intake.interpretation|intake.ask_human|execution.micro|execution.standard] [--shadow-provider claude]` — прогнать repeatable compare corpus через `task_interpret.sh`, `TASK_ASK_COMPARE_ONLY=1 task_ask.sh` или безопасный execution prompt/route plan, записать `provider_corpus_results.jsonl`, `provider_corpus_gate.json` и `provider_corpus_summary.json` в отдельный state-dir; production state и GitHub comments не трогаются, если передан отдельный `--state-dir`.
  - Для transient shadow-provider ошибок (`provider_unavailable`, `provider_rate_limited`, `timeout`) corpus runner делает bounded retry (`--retry-transient`, `--retry-sleep-sec`) и строит gate по latest record per task; полный raw ledger попыток остаётся в `provider_telemetry.jsonl`. Для повторного добора только упавших transient samples используйте тот же `--state-dir` с `--no-clear --rerun-transient-failed` и тем же полным `--issues` corpus set; попытка заменить issue set в существующем state-dir без `--allow-issue-set-rewrite` блокируется.
  - Для `intake.ask_human` можно передать `--ask-fixtures-file <json-or-jsonl>` с records `{issueNumber,kind,message}`; без fixtures runner использует безопасный generic question и всё равно остаётся compare-only.
  - Пример PLANKA ask-human corpus лежит в `.flow/shared/examples/provider-corpus/planka-ask-human-corpus.jsonl`.
  - Для `execution.micro|execution.standard` corpus runner только строит `executor_build_prompt`, canonical route и `execution_plan_*` artifacts; `executor_run.sh`, Codex/Claude execution, git edits, PR и GitHub comments не запускаются.
  - Для `execution.micro` можно явно добавить `--execution-live-smoke`: runner сделает один Claude call без tools/edits по уже собранному executor prompt и запишет `execution_micro_live_smoke_*` artifacts; для `execution.standard` этот режим заблокирован.
- `.flow/shared/scripts/provider_corpus_inspect.sh --state-dir <path> [--module intake.interpretation|intake.ask_human|execution.micro|execution.standard] [--shadow-provider claude]` — показать operator-grade JSON по сохранённому corpus state: gate readiness, blocking reasons, aggregate telemetry/cost и per-issue local/shadow decisions, execution plan prompt hashes, files/kind/action, reasons/body и mismatch reasons.
- `.flow/shared/scripts/provider_corpus_aggregate.sh --state-dir <path> [--state-dir <path> ...]` — объединить несколько сохранённых corpus state-dir в один readiness summary: batch readiness, общий issue set, raw telemetry и latest-per-task gate telemetry totals. Если старый state-dir был случайно перезаписан с неверным `issues` metadata, используйте `--recover-issues-from-telemetry`, чтобы восстановить issue set и gate по latest compare telemetry внутри этого state-dir.
- `.flow/shared/scripts/provider_rollout_gate.sh [--module <module>] [--shadow-provider <provider>] [--min-samples <n>]` — посчитать простой readiness summary для будущего live rollout на основе последних compare telemetry records из `provider_telemetry.jsonl`; сам live switch не делает.
  - Для `intake.interpretation` raw target-file drift остаётся видимым как `targetFilesMismatchCount`, но gate блокирует только `unsafeTargetFilesMismatchRate`; same-profile subset/superset и `blocked` scope drift маркируются как tolerated, а не как full rollout blocker.
  - Для `intake.ask_human` gate дополнительно блокирует unsafe `kindMatch=false` и unsafe `recommendedActionMatch=false`; conservative shadow drift `QUESTION continue -> clarify` маркируется как tolerated, а `optionsMatch=false` остаётся видимой telemetry, но не blocker по умолчанию.
...
- Ownership contract для одного profile:
  - `FLOW_AUTOMATION_RUNTIME_ROLE=authoritative|interactive-only`
  - `FLOW_AUTHORITATIVE_RUNTIME_ID=<runtime-id>`
  - `daemon/watchdog` не трогают очередь в состояниях `INTERACTIVE_ONLY` / `WAIT_RUNTIME_OWNERSHIP`, а install/preflight/status явно показывают ownership summary.
- `.flow/shared/scripts/run.sh remote_agent_access_bootstrap [options]` — legacy bootstrap v1 для repo-backed SSH diagnostics. Штатно отключён; вместо него использовать `remote_agent_v2_bootstrap`.
- `.flow/shared/scripts/run.sh remote_agent_v2_bootstrap [options]` — root-only bootstrap для Remote Agent v2: ставит immutable `gateway/helper/publisher` в `/usr/local`, создает `/etc/ai-flow/public`, `/etc/ai-flow/secrets`, `/var/lib/ai-flow/diagnostics/<profile>`, sudoers fragment и `systemd` timer publisher. SSH enforcement идёт через managed `authorized_keys` forced-command для `aiflow`.
...
- `.flow/shared/scripts/run.sh vpn_safe <command>` — безопасный wrapper для host-level OpenVPN unit. Перед `start/restart` сохраняет `/32` route до текущего SSH client IP через исходный default gateway, чтобы full-tunnel VPN не рвал активную SSH-сессию.
- Для домашнего launcher-а `~/vpn.sh` рекомендуется вызывать сам script напрямую:
