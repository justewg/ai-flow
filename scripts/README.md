...
- `.flow/shared/scripts/provider_corpus_run.sh --issues "633 626 622" [--state-dir <path>] [--module intake.interpretation] [--shadow-provider claude]` — прогнать repeatable compare corpus через `task_interpret.sh`, записать `provider_corpus_results.jsonl`, `provider_corpus_gate.json` и `provider_corpus_summary.json` в отдельный state-dir; production state не трогается, если передан отдельный `--state-dir`.
  - Для transient shadow-provider ошибок (`provider_unavailable`, `provider_rate_limited`, `timeout`) corpus runner делает bounded retry (`--retry-transient`, `--retry-sleep-sec`) и строит gate по latest record per task; полный raw ledger попыток остаётся в `provider_telemetry.jsonl`. Для повторного добора только упавших transient samples используйте тот же `--state-dir` с `--no-clear --rerun-transient-failed`.
...
- `.flow/shared/scripts/provider_corpus_aggregate.sh --state-dir <path> [--state-dir <path> ...]` — объединить несколько сохранённых corpus state-dir в один readiness summary: batch readiness, общий issue set, raw telemetry и latest-per-task gate telemetry totals.
