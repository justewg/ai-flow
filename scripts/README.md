...
- `.flow/shared/scripts/provider_corpus_run.sh --issues "633 626 622" [--state-dir <path>] [--module intake.interpretation|intake.ask_human] [--shadow-provider claude]` — прогнать repeatable compare corpus через `task_interpret.sh` или `TASK_ASK_COMPARE_ONLY=1 task_ask.sh`, записать `provider_corpus_results.jsonl`, `provider_corpus_gate.json` и `provider_corpus_summary.json` в отдельный state-dir; production state и GitHub comments не трогаются, если передан отдельный `--state-dir`.
...
- `.flow/shared/scripts/provider_corpus_inspect.sh --state-dir <path> [--module intake.interpretation|intake.ask_human] [--shadow-provider claude]` — показать operator-grade JSON по сохранённому corpus state: gate readiness, blocking reasons, aggregate telemetry/cost и per-issue local/shadow decisions, files/kind/action, reasons/body и mismatch reasons.
