...
- `.flow/shared/scripts/run.sh remote_agent_access_bootstrap [options]` — optional root-only bootstrap для внешнего AI-agent debug access: создаёт отдельного SSH user (по умолчанию `aiflow`), ставит forced-command gateway, sudoers allowlist только на `remote_probe`, audit log и при желании добавляет один public key в `authorized_keys`.
