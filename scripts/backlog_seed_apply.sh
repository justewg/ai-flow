...
run_gh_retry_capture() {
  local out=""
  local err_file
  err_file="$(mktemp "${STATE_TMP_DIR}/seed_gh_err.XXXXXX")"
...
apply_task_to_project_backlog() {
  ...
  for attempt in 1 2 3 4 5; do
    if set_out="$("${CODEX_SHARED_SCRIPTS_DIR}/project_set_status.sh" "ISSUE-${issue_number}" "Backlog" "Backlog" 2>&1)"; then
      echo "$set_out"
      return 0
    fi
    rc=$?
    ...
    if [[ "$rc" -eq 75 ]]; then
      ...
      runtime_out="$("${CODEX_SHARED_SCRIPTS_DIR}/project_status_runtime.sh" enqueue "ISSUE-${issue_number}" "Backlog" "Backlog" "backlog-seed:${issue_number}" 2>&1 || true)"
      while IFS= read -r line; do
        [[ -z "$line" ]] && continue
        echo "$line"
      done <<< "$runtime_out"
...
while (( processed < MAX_PER_TICK )); do
  ...
  if [[ -z "$code" || -z "$title" || -z "$description" ]]; then
    ...
    tmp_plan="$(mktemp "${STATE_TMP_DIR}/seed_plan.XXXXXX")"
  ...
  if [[ -z "$issue_number" || "$issue_number" == "null" ]]; then
    ...
    body_file="$(mktemp "${STATE_TMP_DIR}/seed_issue_body.XXXXXX")"
