...
usage() {
  cat <<'EOF'
Usage: scripts/codex/profile_init.sh <init|install|preflight|bootstrap> [options]
...
Options:
  ...
  --env-file <path>            Путь к profile env-файлу.
...
Examples:
  scripts/codex/profile_init.sh init --profile acme
  scripts/codex/profile_init.sh install --profile acme
  scripts/codex/profile_init.sh preflight --profile acme
  scripts/codex/profile_init.sh bootstrap --profile acme --dry-run
EOF
}
...
[[ -n "$env_file" ]] || env_file="${ROOT_DIR}/.tmp/codex/profiles/${profile_slug}.env"
[[ -n "$state_dir" ]] || state_dir="${ROOT_DIR}/.tmp/codex/${profile_slug}"
[[ -n "$daemon_label" ]] || daemon_label="com.planka.codex-daemon.${profile_slug}"
[[ -n "$watchdog_label" ]] || watchdog_label="com.planka.codex-watchdog.${profile_slug}"
...
template_contents() {
  cat <<EOF
# Generated by scripts/codex/profile_init.sh for profile ${profile}
# Fill required values before running install/preflight without --dry-run.
...
install_profile() {
  ...
  if [[ "$dry_run" == "1" && ! -f "$env_file" ]]; then
    ...
    printf 'DRY_RUN env DAEMON_GH_ENV_FILE=%s CODEX_STATE_DIR=%s FLOW_STATE_DIR=%s %s %s %s\n' \
      "$env_file" "$state_dir" "$state_dir" "${ROOT_DIR}/scripts/codex/daemon_install.sh" "$daemon_label" "$daemon_interval"
    printf 'DRY_RUN env DAEMON_GH_ENV_FILE=%s CODEX_STATE_DIR=%s FLOW_STATE_DIR=%s WATCHDOG_DAEMON_LABEL=%s WATCHDOG_DAEMON_INTERVAL_SEC=%s %s %s %s\n' \
      "$env_file" "$state_dir" "$state_dir" "$daemon_label" "$daemon_interval" "${ROOT_DIR}/scripts/codex/watchdog_install.sh" "$watchdog_label" "$watchdog_interval"
  ...
  local daemon_cmd=("${ROOT_DIR}/scripts/codex/daemon_install.sh" "$daemon_label" "$daemon_interval")
  local watchdog_cmd=("${ROOT_DIR}/scripts/codex/watchdog_install.sh" "$watchdog_label" "$watchdog_interval")
...
preflight_profile() {
  ...
  if [[ "$dry_run" == "1" && ! -f "$env_file" ]]; then
    ...
    cat <<EOF
SMOKE_STEP 1 scripts/codex/run.sh profile_init init --profile ${profile} --env-file ${env_file} --state-dir ${state_dir}
SMOKE_STEP 2 fill required env in ${env_file}
SMOKE_STEP 3 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh profile_init install --profile ${profile}
SMOKE_STEP 4 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh github_health_check
SMOKE_STEP 5 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh status_snapshot
EOF
  ...
  daemon_status="$(status_summary "$daemon_label" "${ROOT_DIR}/scripts/codex/daemon_status.sh")"
  watchdog_status="$(status_summary "$watchdog_label" "${ROOT_DIR}/scripts/codex/watchdog_status.sh")"
  ...
  cat <<EOF
SMOKE_STEP 1 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh daemon_status ${daemon_label}
SMOKE_STEP 2 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh watchdog_status ${watchdog_label}
SMOKE_STEP 3 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh github_health_check
SMOKE_STEP 4 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/run.sh status_snapshot
SMOKE_STEP 5 env DAEMON_GH_ENV_FILE=${env_file} CODEX_STATE_DIR=${state_dir} FLOW_STATE_DIR=${state_dir} scripts/codex/gh_app_auth_token.sh >/dev/null
EOF
