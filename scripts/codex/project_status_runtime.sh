...
usage() {
  cat <<'EOF'
Usage:
  scripts/codex/project_status_runtime.sh enqueue <target> <status> [flow] [reason]
  scripts/codex/project_status_runtime.sh apply [max-per-run]
  scripts/codex/project_status_runtime.sh list
  scripts/codex/project_status_runtime.sh clear
EOF
}
...
enqueue_action() {
  ...
  tmp="$(mktemp "${CODEX_DIR}/status_runtime.XXXXXX")"
  jq \
    --arg target "$target" \
...
apply_queue() {
  ...
  while (( processed < pass_limit )); do
    ...
    if set_out="$("${ROOT_DIR}/scripts/codex/project_set_status.sh" "$target" "$status_name" "$flow_name" 2>&1)"; then
