...
(`CODEX_STATE_DIR` or `FLOW_STATE_DIR`, default `.tmp/codex`):
  pr_number.txt
  pr_title.txt
  pr_body.txt
  commit_message.txt
...
case "$cmd" in
  ...
  ops_bot_webhook_register)
    if [[ $# -gt 2 ]]; then
      echo "Usage: scripts/codex/run.sh ops_bot_webhook_register [register|refresh|delete|info]"
      exit 1
    fi
  ...
  ops_bot_webhook_refresh)
    if [[ $# -ne 1 ]]; then
      echo "Usage: scripts/codex/run.sh ops_bot_webhook_refresh"
      exit 1
    fi
  ...
  ops_remote_summary_push)
    if [[ $# -ne 1 ]]; then
      echo "Usage: scripts/codex/run.sh ops_remote_summary_push"
      exit 1
    fi
    "${ROOT_DIR}/scripts/codex/ops_remote_summary_push.sh"
    ;;
