...
5) Если нужен вопрос пользователю или есть блокер:
   - пользовательский вопрос допустим только когда реально нужен новый факт, выбор или решение по scope;
   - внутренние ошибки, строки логов, строки кода, diff-фрагменты, sample/event lines и git-status строки не являются поводом для USER_REPLY;
   - если вызываешь blocker, файл обязан быть структурирован:
     * ASK_REASON=needs_user_fact|needs_user_decision|needs_scope_decision
     * ASK_QUESTION=<один явный вопрос пользователю>
     * ASK_CONTEXT=<краткий контекст> (опционально)
   - без этих полей blocker будет автоматически отклонён как malformed;
   - выполни: .flow/shared/scripts/run.sh task_ask question <message-file> (или blocker);
   - после публикации вопроса/блокера больше не продолжай рабочие итерации: executor должна остановиться и ждать ответа.
...
- Не проси USER_REPLY, если единственный разумный ответ был бы "продолжай".
