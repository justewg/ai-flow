...
PAYLOAD_DIR="$(codex_resolve_state_outbox_payload_dir "$CODEX_DIR")"
FAILED_DIR="$(codex_resolve_state_outbox_failed_dir "$CODEX_DIR")"
...
usage() {
  cat <<'USAGE'
Usage:
  .flow/shared/scripts/github_outbox.sh enqueue_issue_comment <repo> <issue-number> <body-file> [task-id] [kind] [set-waiting]
  .flow/shared/scripts/github_outbox.sh flush
  .flow/shared/scripts/github_outbox.sh count
  .flow/shared/scripts/github_outbox.sh list
USAGE
}
...
case "$cmd" in
  ...
  flush)
    ...
    for item_file in $(printf '%s\n' "${items[@]}" | sort); do
      ...
      api_out=""
      err_file="$(mktemp "${STATE_TMP_DIR}/outbox_gh_err.XXXXXX")"
      if api_out="$("${CODEX_SHARED_SCRIPTS_DIR}/gh_retry.sh" gh api "repos/${repo}/issues/${issue_number}/comments" -f body="$body_text" 2>"$err_file")"; then
        if [[ -s "$err_file" ]]; then
        ...
        if [[ "$rc" -eq 75 ]]; then
          ...
          attempts=$((attempts + 1))
          tmp_item="$(mktemp "${STATE_TMP_DIR}/outbox_item.XXXXXX")"
...
mkdir -p "$STATE_TMP_DIR" "$OUTBOX_DIR" "$PAYLOAD_DIR" "$FAILED_DIR"
