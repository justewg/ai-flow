...
mkdir -p "${FLOW_TMP_DIR}" "${CODEX_DIR}"
...
run_gh_retry_capture() {
  local out=""
  local rc=0
  if out="$("${CODEX_SHARED_SCRIPTS_DIR}/gh_retry.sh" "$@" 2>&1)"; then
    printf '%s' "$out"
    return 0
  fi
  rc=$?
...
ensure_project_backlog() {
  ...
  if out="$("${CODEX_SHARED_SCRIPTS_DIR}/project_set_status.sh" "ISSUE-${issue_number}" "Backlog" "Backlog" 2>&1)"; then
    :
  else
    rc=$?
    if [[ "$rc" -eq 75 ]] || printf '%s' "$out" | grep -Eiq 'rate limit|api\.github\.com|could not resolve|timeout|connection reset|failed to connect'; then
      "${CODEX_SHARED_SCRIPTS_DIR}/project_status_runtime.sh" enqueue "ISSUE-${issue_number}" "Backlog" "Backlog" "issue-285-reframe:${issue_number}" >/dev/null 2>&1 || true
...
tmp_manual_body="$(mktemp "${STATE_TMP_DIR}/issue285.manual.XXXXXX.md")"
tmp_auto_body="$(mktemp "${STATE_TMP_DIR}/issue285.auto.XXXXXX.md")"
tmp_post_body="$(mktemp "${STATE_TMP_DIR}/issue285.post.XXXXXX.md")"
tmp_reframed_285="$(mktemp "${STATE_TMP_DIR}/issue285.reframed.XXXXXX.md")"
...
4. Зарегистрировать webhook:
   - через wrapper: `.flow/shared/scripts/run.sh ops_bot_webhook_register register`
   - напрямую: `.flow/shared/scripts/ops_bot_webhook_register.sh register`
   - только проверка: `.flow/shared/scripts/ops_bot_webhook_register.sh info`
...
- reload nginx;
- остановить ops bot: `.flow/shared/scripts/run.sh ops_bot_pm2_stop`.
...
3. Проверить устойчивость:
   - отсутствие падений на пустых/частично отсутствующих `.flow/state/*` и `.flow/tmp/*`;
