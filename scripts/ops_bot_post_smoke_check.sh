...
SERVICE_FILE="${CODEX_SHARED_SCRIPTS_DIR}/ops_bot_service.js"

manual_file="${FLOW_TMP_DIR}/issue_285_manual_enriched.md"
report_file="${FLOW_TMP_DIR}/issue_316_post_smoke_report.md"
write_report="1"
...
usage() {
  cat <<'EOF'
Usage:
  .flow/shared/scripts/ops_bot_post_smoke_check.sh [--manual-file <path>] [--report-file <path>] [--no-report]
...
check_webhook_info() {
  ...
  set +e
  output="$("${CODEX_SHARED_SCRIPTS_DIR}/ops_bot_webhook_register.sh" info 2>&1)"
  rc=$?
  set -e
