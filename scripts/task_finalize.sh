...
smoke_check_branch_mismatch() {
  ...
  echo "FLOW_SMOKE_BRANCH_INVALID=0"
  echo "FLOW_SMOKE_CURRENT_BRANCH=${current_branch}"
  echo "FLOW_SMOKE_HEAD_BRANCH=${HEAD_BRANCH}"
  echo "FLOW_SMOKE_BASE_BRANCH=${BASE_BRANCH}"
  return 0
}
...
EOF
  ...
  local comment_json=""
  local err_file
  err_file="$(mktemp "${STATE_TMP_DIR}/final_comment_gh_err.XXXXXX")"
  if comment_json="$(
    "${CODEX_SHARED_SCRIPTS_DIR}/gh_retry.sh" \
    ...
    if [[ "$rc" -eq 75 ]]; then
      ...
      tmp_body="$(mktemp "${STATE_TMP_DIR}/in_review_comment.XXXXXX")"
...
if (( open_pr_count == 0 )); then
  ...
  "${CODEX_SHARED_SCRIPTS_DIR}/pr_edit.sh" "$pr_number" "$tmp_title" "$tmp_body" >/dev/null
...
if [[ -n "$review_issue_number" && -z "$review_comment_id" && "$review_pending_post" != "1" ]]; then
  ...
  recover_body_file="$(mktemp "${STATE_TMP_DIR}/review_anchor_recover.XXXXXX")"
...
if final_status_out="$("${CODEX_SHARED_SCRIPTS_DIR}/project_set_status.sh" "$status_target" "$FINAL_STATUS" "$FINAL_FLOW" 2>&1)"; then
  ...
  if [[ "$final_status_rc" -eq 75 ]] || printf '%s' "$final_status_out" | grep -Eiq \
    ...
    runtime_out="$("${CODEX_SHARED_SCRIPTS_DIR}/project_status_runtime.sh" enqueue "$status_target" "$FINAL_STATUS" "$FINAL_FLOW" "task-finalize:${task_id}" 2>&1 || true)"
