...
TICK_LOCK_DIR="${CODEX_DIR}/watchdog_tick.lock"
...
acquire_tick_lock() {
  ...
  if [[ "$owner_pid" =~ ^[0-9]+$ ]] && kill -0 "$owner_pid" 2>/dev/null; then
    echo "WATCHDOG_TICK_ALREADY_RUNNING=1"
    echo "WATCHDOG_TICK_LOCK_OWNER_PID=${owner_pid}"
    return 1
  fi
  ...
  if mkdir "$TICK_LOCK_DIR" 2>/dev/null; then
    printf '%s\n' "$$" > "$TICK_LOCK_OWNER_FILE"
    echo "WATCHDOG_TICK_STALE_LOCK_RECOVERED=1"
    [[ -n "$owner_pid" ]] && echo "WATCHDOG_TICK_STALE_LOCK_OWNER_PID=${owner_pid}"
    return 0
  fi

  echo "WATCHDOG_TICK_LOCK_BUSY=1"
  return 1
}
...
notify_action() {
  ...
  local detail="$3"
  local msg_file
  msg_file="$(mktemp "${STATE_TMP_DIR}/watchdog_notify.XXXXXX")"
...
EOF
  local out rc
  if out="$("${CODEX_SHARED_SCRIPTS_DIR}/telegram_local_notify.sh" "$msg_file" 2>&1)"; then
    while IFS= read -r line; do
      [[ -z "$line" ]] && continue
      log "TG_NOTIFY: $line"
    done <<< "$out"
